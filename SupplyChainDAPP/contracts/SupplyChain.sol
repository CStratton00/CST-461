...
contract supplyChain {
    ...
    function createOwner(uint32 _userId, uint32 _prodId) public returns (bool) {
        participant memory p1 = participants[_userId];
        uint32 ownership_id = owner_id++;

        if(keccak256(abi.encodePacked(p1.participantType)) == keccak256("Manufacturer")){
            ownerships[ownership_id].productId = _prodId;
            ownerships[ownership_id].productOwner = p1.participantAddress;
            ownerships[ownership_id].ownerId = _userId;
            ownerships[ownership_id].trxTimeStamp = uint32(now);
            products[_prodId].productOwner = p1.participantAddress;
            productTrack[_prodId].push(ownership_id);
            emit TransferOwnership(_prodId);
